...
def load_data():
    ...
    try:
        ...
        st.error("Failed to load data from GitHub. Using sample data instead.")
...
# Define feature columns (matching the GitHub dataset)
...
# Main Content - Visualizations (equal column width for same size graphs)
col1, col2 = st.columns(2)  # Changed from [2, 1] to equal width

with col1:
    st.subheader("Scatter Plot")

    # Create scatter plot with standardized size
    fig1, ax1 = plt.subplots(figsize=(8, 6))  # Explicit figure size
    sns.scatterplot(
        data=filtered_df,
        x=x_axis,
        y=y_axis,
    ...
    # Move legend inside the plot for better space utilization
...
with col2:
    ...
    # Create histogram with same standardized size
    fig2, ax2 = plt.subplots(figsize=(8, 6))  # Same figure size as scatter plot
